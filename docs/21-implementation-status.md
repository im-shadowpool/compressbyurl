...
| 36. Website Audit & Replacement Workflow | Complete | Validated supported-image audit UI with automatic three-at-a-time verification, visible previews, URL/count/total-size/estimated-savings summary, default all-selection, source/size/format/natural-dimension facts, issue and duplicate-variant tags, local batch optimization/compare, and a verified replacement ZIP containing `replacement-map.json`. |
...
- Website audit: supported static images are verified and measured three at a time, displayed with thumbnails, selected by default, and summarized by URL, total bytes, and estimated WebP-at-90% savings before local compression
...
- Settings dropdown polish: each settings pill now opens an anchored dropdown panel in the Popover API top layer (flipped above when space is tight, clamped to the viewport) with a two-way fade-and-slide, chevron rotation, outside-click and Escape dismissal, keyboard focus entry, and no Done button; browsers without the Popover API fall back to a fixed-position panel with the same transitions
- Homepage canvas layout: the dark hero frame now spans the viewport minus a small edge gutter through a new `bleed` container (instead of the fixed 80rem column), and the tool is nested inside that canvas as a centered white box (`max-width: 88rem`, dark-canvas padding around it, depth shadow) rather than overlapping it from below; inner spacing follows a `--tool-padding` rhythm, and URL mode forms are capped at 56rem so inputs stay readable on wide screens
- Settings dropdown polish: fields were compacted (40px inputs and select triggers, 36px segmented options, 13px labels, 16px panel rhythm, 356px panel width) and the native `<select>` was replaced with a custom listbox combobox built on the new shared `useAnchoredPopover` hook — complete with check-on-selected options, arrow/Home/End/Enter/Escape handling, `aria-activedescendant`, hover-active sync, and the same open/close fade-and-slide as the settings panel
- URL mode alignment: the Image URL and Website URL stages now use the same centered composition as the upload dropzone — centered icon/title/description intro, centered form capped at 52rem, centered privacy note — and the submit button is top-aligned with its input (was bottom-aligned with the hint text) at a matching 48px height; the website audit results and URL import workspace are centered at 72rem for readable line lengths
